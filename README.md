# reF1nd Sing-Box Docker

自动构建支持统一 TC eBPF 入站的 Stable 与 Testing 多架构镜像。

## 镜像

- Stable：`ghcr.io/cary17/sing-box:latest` / `cary17/sing-box:latest`
- Testing：`ghcr.io/cary17/sing-box:testing` / `cary17/sing-box:testing`

支持 `amd64`、`arm64`、`386`、`arm/v7`、`arm/v6`。镜像使用纯 Go、`CGO_ENABLED=0` 和 `with_ebpf` 构建。

## 使用

将 sing-box 配置放入 `/opt/sing-box/conf`，然后使用普通 TUN Compose：

```bash
mkdir -p /opt/sing-box/conf
docker compose up -d
docker compose exec sing-box sing-box version
docker compose logs --tail 100 sing-box
```

使用 eBPF Compose：

```bash
docker compose -f docker-compose.ebpf.yml up -d
docker compose -f docker-compose.ebpf.yml exec sing-box sing-box version
docker compose -f docker-compose.ebpf.yml exec sing-box sing-box tools ebpf status --mode local
```

版本输出应包含 `with_ebpf` 和 `CGO: disabled`；eBPF 是否可用以 `tools ebpf status` 的探测结果为准。

更新镜像：

```bash
docker compose pull
docker compose up -d
```

生产环境可使用摘要固定镜像：

```yaml
image: ghcr.io/cary17/sing-box@sha256:<manifest-digest>
```

GitHub Actions 会自动检测 Stable/Testing 新版本；手动运行时可选择渠道并使用 `force_build` 强制构建。构建记录位于 `.github/version/`。
